...
从上面传统I/O流程中，可以看到一次`read`或`write`调用，需要两次数据拷贝，两次上下文切换，其中一次拷贝是`DMA`操作, 故完整一次I/O流程需要**两次CPU拷贝，两次DMA拷贝，四次上下文切换** 。如果使用`mmap`后，只需要**一次CPU拷贝（从文件拷贝到用户空间与内核空间的共享缓冲区中），两次DMA拷贝，四次上下文切换**，相同传统I/O操作少了一次CPU拷贝。这种技术也称为**零拷贝（Zero-Copy）技术**。零拷贝是一种高效的数据传输技术，旨在减少或消除数据在传输过程中的拷贝次数，从而提高数据传输效率和系统性能。
